(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("4ab094b5-c4f3-43ca-ab53-80a87aadfc1d");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '4ab094b5-c4f3-43ca-ab53-80a87aadfc1d' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js": "T2yuo9Oe71Cz/I4X9Ac5+gpEa5a8PpJCDlqKYO0CfAuEszu1JrXLl8YugMqYe3sM", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js": "98GDGJ0kOMCUMUePhksaQ/GYgB3+NH9h996V88sh3aOiUNX3N+fLXAtry6xctSZ6", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js": "89bArO+nlbP3sgakeHjCo1JYxYR5wufVgA3IbUvDY+K7w4zyxJqssu7wVnfeKCq8"};
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"f5c3316c-acce-4d10-b4e5-a5946e7614ae":{"roots":{"references":[{"attributes":{},"id":"14767","type":"SaveTool"},{"attributes":{},"id":"14793","type":"BasicTickFormatter"},{"attributes":{"toolbar":{"id":"14826"},"toolbar_location":"above"},"id":"14827","type":"ToolbarBox"},{"attributes":{},"id":"14822","type":"Selection"},{"attributes":{"toolbars":[{"id":"14771"}],"tools":[{"id":"14761"},{"id":"14762"},{"id":"14763"},{"id":"14764"},{"id":"14765"},{"id":"14766"},{"id":"14767"},{"id":"14768"}]},"id":"14826","type":"ProxyToolbar"},{"attributes":{"data_source":{"id":"14780"},"glyph":{"id":"14781"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14782"},"selection_glyph":null,"view":{"id":"14784"}},"id":"14783","type":"GlyphRenderer"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"14805","type":"Circle"},{"attributes":{},"id":"14745","type":"DataRange1d"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14786","type":"Line"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"xs_units":"screen","ys_units":"screen"},"id":"14770","type":"PolyAnnotation"},{"attributes":{"items":[{"id":"14803"}]},"id":"14802","type":"Legend"},{"attributes":{},"id":"14758","type":"BasicTicker"},{"attributes":{},"id":"14751","type":"LinearScale"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"14761"},{"id":"14762"},{"id":"14763"},{"id":"14764"},{"id":"14765"},{"id":"14766"},{"id":"14767"},{"id":"14768"}]},"id":"14771","type":"Toolbar"},{"attributes":{},"id":"14762","type":"PanTool"},{"attributes":{},"id":"14798","type":"Selection"},{"attributes":{},"id":"14754","type":"BasicTicker"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"14788"}]},"id":"14803","type":"LegendItem"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14781","type":"Line"},{"attributes":{"source":{"id":"14785"}},"id":"14789","type":"CDSView"},{"attributes":{"source":{"id":"14804"}},"id":"14808","type":"CDSView"},{"attributes":{},"id":"14800","type":"Selection"},{"attributes":{"children":[[{"id":"14744"},0,0]]},"id":"14825","type":"GridBox"},{"attributes":{},"id":"14747","type":"DataRange1d"},{"attributes":{"text":"y","text_font_size":{"value":"15pt"}},"id":"14809","type":"Title"},{"attributes":{},"id":"14764","type":"WheelZoomTool"},{"attributes":{"axis":{"id":"14757"},"dimension":1,"ticker":null},"id":"14760","type":"Grid"},{"attributes":{"source":{"id":"14780"}},"id":"14784","type":"CDSView"},{"attributes":{"callback":null},"id":"14768","type":"HoverTool"},{"attributes":{"data":{},"selected":{"id":"14800"},"selection_policy":{"id":"14801"}},"id":"14785","type":"ColumnDataSource"},{"attributes":{"below":[{"id":"14753"}],"center":[{"id":"14756"},{"id":"14760"},{"id":"14802"}],"left":[{"id":"14757"}],"output_backend":"webgl","plot_height":500,"plot_width":500,"renderers":[{"id":"14783"},{"id":"14788"},{"id":"14807"}],"title":{"id":"14809"},"toolbar":{"id":"14771"},"toolbar_location":null,"x_range":{"id":"14745"},"x_scale":{"id":"14749"},"y_range":{"id":"14747"},"y_scale":{"id":"14751"}},"id":"14744","subtype":"Figure","type":"Plot"},{"attributes":{"overlay":{"id":"14769"}},"id":"14763","type":"BoxZoomTool"},{"attributes":{},"id":"14801","type":"UnionRenderers"},{"attributes":{"formatter":{"id":"14793"},"ticker":{"id":"14758"}},"id":"14757","type":"LinearAxis"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"line_alpha":{"value":0.1},"size":{"units":"screen","value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"14806","type":"Circle"},{"attributes":{},"id":"14766","type":"UndoTool"},{"attributes":{},"id":"14795","type":"BasicTickFormatter"},{"attributes":{"data":{},"selected":{"id":"14822"},"selection_policy":{"id":"14823"}},"id":"14804","type":"ColumnDataSource"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14782","type":"Line"},{"attributes":{"data_source":{"id":"14804"},"glyph":{"id":"14805"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14806"},"selection_glyph":null,"view":{"id":"14808"}},"id":"14807","type":"GlyphRenderer"},{"attributes":{},"id":"14799","type":"UnionRenderers"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"14798"},"selection_policy":{"id":"14799"}},"id":"14780","type":"ColumnDataSource"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14787","type":"Line"},{"attributes":{"overlay":{"id":"14770"}},"id":"14765","type":"LassoSelectTool"},{"attributes":{},"id":"14761","type":"ResetTool"},{"attributes":{"axis":{"id":"14753"},"ticker":null},"id":"14756","type":"Grid"},{"attributes":{},"id":"14823","type":"UnionRenderers"},{"attributes":{},"id":"14749","type":"LinearScale"},{"attributes":{"formatter":{"id":"14795"},"ticker":{"id":"14754"}},"id":"14753","type":"LinearAxis"},{"attributes":{"children":[{"id":"14827"},{"id":"14825"}]},"id":"14828","type":"Column"},{"attributes":{"data_source":{"id":"14785"},"glyph":{"id":"14786"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14787"},"selection_glyph":null,"view":{"id":"14789"}},"id":"14788","type":"GlyphRenderer"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","top_units":"screen"},"id":"14769","type":"BoxAnnotation"}],"root_ids":["14828"]},"title":"Bokeh Application","version":"2.2.3"}}';
                  var render_items = [{"docid":"f5c3316c-acce-4d10-b4e5-a5946e7614ae","root_ids":["14828"],"roots":{"14828":"4ab094b5-c4f3-43ca-ab53-80a87aadfc1d"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();